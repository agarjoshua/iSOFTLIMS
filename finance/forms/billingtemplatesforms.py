...
from finance.models import BillingTemplate, CurriculumSystem, BillingItem, Program, School
...
class BillingTemplateForm(forms.ModelForm): 
    ...
    # billing_item = forms.ModelChoiceField(label="Billing Item", queryset=BillingItem.objects.all(), widget=forms.Select(attrs={"class": "form-control"}), required=False)
...
class EditBillingTemplateForm(forms.ModelForm):
    ...
    fee_item = forms.ModelChoiceField(label="Fee Item", queryset=BillingItem.objects.all(), widget=forms.Select(attrs={"class": "form-control"}), required=False)
